 ...
 */
public class TokenMediatedIndexerWriter<T extends BaseModel<?>>
	implements IndexerWriter<T> {

	public TokenMediatedIndexerWriter(
		BaseModelRetriever baseModelRetriever,
		IndexerTokenFactory indexerTokenFactory,
		IndexerTokenConsumer indexerTokenConsumer,
	...
	public void delete(long companyId, String uid) {
		...
		_indexerTokenConsumer.consume(createDeleteIndexerToken(companyId, uid));
	}
	...
	public void reindex(String[] ids) {
		...
		for (String id : ids) {
			...
			_indexerTokenConsumer.consume(indexerToken);
		}
	}
	...
	public void reindex(T baseModel) {
		...
		if ((indexerWriterMode == IndexerWriterMode.UPDATE) ||
			...
			_indexerTokenConsumer.consume(
				createReindexModelIndexerToken(baseModel));
		}
	...
	public void updatePermissionFields(T baseModel) {
		_indexerTokenConsumer.consume(
			createUpdatePermissionFieldsIndexerToken(baseModel));
	}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		TokenMediatedIndexerWriter.class);

	private final BaseModelRetriever _baseModelRetriever;
	private boolean _enabled;
	private final IndexerDocumentBuilder _indexerDocumentBuilder;
	private final IndexerTokenConsumer _indexerTokenConsumer;
