...
public class IndexerTokenConsumer {
	...
	public IndexerTokenConsumer(BaseModelRetriever baseModelRetriever,
	...
	protected void executeDelete(IndexerToken indexerToken) {
		try {
			_indexWriterHelper.deleteDocument(indexerToken.getSearchEngineId(),
					indexerToken.getCompanyId(), indexerToken.getUid(),
					indexerToken.isCommitImmediately());
		} catch (SearchException se) {
			_log.error("Failed to delete document (uid=" + indexerToken.getUid() +
				") from index.", se);
		}
	}
	...
	protected void executeReindexModel(IndexerToken indexerToken) {
		...
		baseModelOptional.ifPresent(baseModel -> reindex(
	...
	protected Optional<BaseModel<?>> getBaseModelOptional(
		...
		return _baseModelRetriever.fetchBaseModel(indexerToken.getClassName(),
			classPK);
	}
	...
	protected void reindex(String searchEngineId, long companyId,
			BaseModel<?> baseModel, boolean commitImmediately) {

		Document document = getDocument(baseModel);

		_updateDocumentIndexWriter.updateDocument(searchEngineId, companyId,
			document, commitImmediately);
	}

	protected void updatePermissionFields(String searchEngineId, long companyId,
			BaseModel<?> baseModel, boolean commitImmediately) {

		_searchPermissionIndexWriter.updatePermissionFields(baseModel,
			companyId, searchEngineId, commitImmediately);
	}

	private static final Log _log = LogFactoryUtil
		.getLog(IndexerTokenConsumer.class);
