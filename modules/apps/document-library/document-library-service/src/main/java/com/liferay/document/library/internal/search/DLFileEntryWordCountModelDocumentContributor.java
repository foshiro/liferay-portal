...
public class DLFileEntryWordCountModelDocumentContributor
	...
	public void contribute(Document document, DLFileEntry dlFileEntry) {
		...
		try {
			inputStreamOptional.ifPresent(
				is -> {
					try {
						document.addNumber("wordCount", countWords(is));
					}
			...
			_log.debug(
				"Document " + dlFileEntry + " word count indexed successfully");
		}
		catch (Exception e) {
	...
	private static final Log _log = LogFactoryUtil.getLog(
		DLFileEntryWordCountModelDocumentContributor.class);
